...
The current [PDF to handwriting converter](/tools/text-to-handwriting-pdf) accepts typed or pasted text and can extract selectable text from a text-based PDF in the browser. It does not preserve the source layout or run OCR, so scanned files still need separate OCR and proofreading.
