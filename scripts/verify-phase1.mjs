...
for (const event of ["tool_view", "editor_focus", "preview_rendered", "preview_error", "pdf_import_started", "pdf_import_completed", "pdf_import_error", "export_started", "export_completed", "export_error", "related_tool_clicked", "guide_clicked", "template_downloaded"]) {
...
const eventCalls = [...`${tool}\n${importer}`.matchAll(/trackEvent\([\s\S]{0,220}?\);/g)].map((match) => match[0]).join("\n");
...
assert.match(importer, /OCR support will be added separately/);
